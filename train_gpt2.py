...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets = None):
        B, T = idx.size()
        assert T <= self.config.block_size, f"Cannot forced sequences of length {T}, block size is {self.config.block_size}"
    ...
    def from_pretrained(cls, model_type):
        ...
        # ignoring some unneccessary layers, transposing some layers acc to pytorch
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device_type):
        ...
        num_no_decay_params = sum(p.numel() for p in non_decay_params)
        if master_process:
            print(f"num decayed parameter tensors: {len(decay_params)}, with {num_decay_params:,} parameters")
            print(f"num non-decayed parameter tensors: {len(non_decay_params)}, with {num_no_decay_params:,} parameters")
...
if ddp:
    assert torch.cuda.is_available(), "CUDA should be available for DDP"
    ...
    master_process = ddp_rank == 0 # for logging and checkpointing
else:
    ddp_rank = 0
    ddp_local_rank = 0
    ddp_world_size = 1
...
model.to(device)
use_compile = False  # set True only for single-GPU debug runs
if use_compile and not ddp:
...
for step in range(max_steps):
    ...
    if master_process:
        print(f"step {step} | loss: {loss_accum.item()} | lr: {lr:.4e} | norm: {norm:.4f} | dt:{dt*1000:.2f}ms | tok/s: {tok_per_sec:.2f}")
